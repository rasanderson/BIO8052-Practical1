---
title: "BIO8052 Quantitative Methods"
...
R is open source software, freely available to download from the main website if you wish to install it on your own PC (it runs on Windows, Macs and Linux):
...
This will start up the RStudio program.  If you get a pop-up box asking you whether to start the 64-bit or 32-bit version, use the 32-bit version.  You should see the following screen:

![RStudio screen](images/RStudio_screen.jpg)
...
RStudio needs to know where to look to find data, or where to export data.  It is best to create a folder for any particular analysis, and store R-scripts, data-files, results etc in that folder.  So before you begin your analyses, create a folder BIO8052 in your Documents Filespace and within that folder create a subfolder called Practical 1 
...
# R packages
R has many 'add-on' packages that greatly extend its capabilities.  In this module we are going to make use of the 'tidyverse' package, which is a group of packages (or meta-package) providing facilities for advanced plotting of data (ggplot), data manipulation (tidyr) etc.  To install the package type:

`install.packages("tidyverse")`

as it is a big collection of packages it may take a while to install.  Once installed you can activate it with the library function (don't worry about warning messages):
...
![Windows Explorer csv and xlsx files](images/csv_vs_xlsx.jpg)
...
![Environment window](images/environ_window.jpg)
...
__Note:__ you may notice many books about R using the `read.csv` command rather than `read_csv` function.  The latter is more advanced, and recommended for this module.
...
Suppose you want to calculate the average organic matter, and standard deviation, for each site.  This is easy to do using the summarise and group_by functions, and also a "pipe" symbol %>% which sends data or the output of one function into another function:

```{r group_by and summarise example}
orgmatt.dat %>%
   group_by(site) %>%
...
```{r group_by and summarise saving results}
...
# Plotting your data
In this module we are mainly going to use one of the 'tidyverse' packages, ggplot2, to undertake plotting.  Although it is harder to learn than standard base plots, it gives you much more control over appearance, layout etc.  Again, it is usually best to store the code for your plots in your R Script file; that means you can regenerate them exactly whenever you need, even if your original data change.
...
There are several different themes (some used by UK national newspapers or scientific journals) available in ggplot.  I often use `theme_bw()` and `theme_classic()`
...
Your response and explanatory data are now in two columns, and this type of data is often called 'tidy data' as it is usually easier to analyse and plot.

__On your own:__

* Produce summary statistics (mean, sd) for your insect.tidy data (Hint: use the `group_by`, `summarise` and `%>%` functions.)
* Produce a boxplot of your insect diversity data (Hint: build up your boxplot in ggplot layer-by-layer in your R Script as you did earlier).
...
Re-run the ggplot code to produce your boxplot; you will now see that the order of the three treatments is more logical.  In general, after reading your data into R using read_csv you may want to convert character variables to factors if they represent experimental treatments (leave them as characters otherwise).
...
```{r read bluetit data}
bluetit.dat <- read_csv("data/bluetit.csv")
summary(bluetit.dat)
```
...
You can see that there is a general trend of increased bird condition with the provision of garden food, although the data are noiser at the upper end of the scale.  We will fit a statistical model to these data in a later practical, but we can easily get an overview with ggplot:

```{r ggplot bluetit scatter plus lm}
ggplot(bluetit.dat, aes(x=gardenfood, y=condition)) +
    geom_point() +
...
These are not automatically calculated by R, but I have added a simple script `ci95.R` onto the Blackboard website.  Download and save this script in your Practical 1 folder.  The script creates a function, `ci95` that you can use to display Confidence Intervals.  It is a little more complicated than the one you have just created for standard errors, so ask if you do not understand any of it.  Open the script in RStudio by clicking on __File -> Open File__ from its main menu.  Next, push the commands into R by clicking on the __Source__ button at the top right of the R Script window.

This then creates an `ci95` function to calculate 95% CI which you can use in the usual way to find the relevant values for the three cutting regimes.
...
```{r export to csv}
write_csv(insect.tidy, "data/insect_tidy.csv")
```
...
* Example - shows how the function can be used, often with the aid of a built-in dataset.  The example section can be particularly useful to help understand functions.  You can even run an example:
...
In the above output, the commands listed in the example at the end of the manual page for the mean function are run.  Note that I added the option `local=TRUE` to ensure that none of the variables created in the example interfered with your own workspace.  If you run the example without this option (the default) you will notice that the example datasets x and xm will appear in your workspace.  There are also a lot of excellent tutorials and introductory videos on R usage on the internet which are worth exploring.
